...
type Dict[K comparable, V] struct { // Dict genérico con K comparable (requisito, hay types que no son comparables en golang) y V cualquiera
...
func NewDict[K comparable, V any]() *Dict[K, V] { // Constructor del diccionario
	return &Dict[K, V]{m: make(map[K]V)} // Inicializamos el map con make y devolvemos el puntero al Dict
}
